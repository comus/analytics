...
module.exports.request = function (url) {
  ...
  if (url.agent) { agent = url.agent } else if (url.agent === false && !ipv6) { agent = new Agent({ type: 'udp4' }) } else if (url.agent === false && ipv6) { agent = new Agent({ type: 'udp6' }) } else if (ipv6) { agent = exports.globalAgentIPv6 } else { agent = exports.globalAgent }
