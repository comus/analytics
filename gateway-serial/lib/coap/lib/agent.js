...
Agent.prototype._doClose = function () {
  for (var k in this._msgIdToReq) { this._msgIdToReq[k].sender.reset() }
...
Agent.prototype._handle = function handle (msg, rsinfo, outSocket) {
  ...
  if (packet.code == '0.00' && !packet.reset) { return }
...
Agent.prototype.request = function request (url) {
  ...
  var option
  var that = this
  var multicastTimeout = url.multicastTimeout !== undefined ? parseInt(url.multicastTimeout) : 20000
  ...
  if (url.observe) { req.setOption('Observe', null) } else { req.on('response', this._cleanUp.bind(this)) }
