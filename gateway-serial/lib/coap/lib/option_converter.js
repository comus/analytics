...
var contentFormatToString = function (value) {
  ...
  if (value.length === 1) { value = value.readUInt8(0) } else if (value.length === 2) { value = value.readUInt16BE(0) } else { throw new Error('Content-Format option is too big') }

  var result = formatsBinaries[value]

  if (!result) { throw new Error('No matching format found') }
