...
var lowerCodeMask = 31

var nextMsgId = Math.floor(Math.random() * 65535)
...
module.exports.generate = function generate (packet) {
  ...
  if (length > 1280) { throw new Error('Max packet size is 1280: current is ' + length) }
  ...
  if (codes[packet.code]) { buffer.writeUInt8(codes[packet.code], pos++) } else { buffer.writeUInt8(toCode(packet.code), pos++) }
...
module.exports.parse = function parse (buffer) {
  ...
  if (result.code !== '0.00') {
    ...
    if (buffer.length != 4) { throw new Error('Empty messages must be empty') }
...
function parseCode (buffer) {
  ...
  if (byte < 10) { code += '0' + byte } else { code += byte }
...
function fillGenDefaults (packet) {
  ...
  if (!packet.payload) { packet.payload = empty }

  if (!packet.token) { packet.token = empty }

  if (packet.token.length > 8) { throw new Error('Token too long') }

  if (!packet.code) { packet.code = '0.01' }

  if (!packet.messageId) { packet.messageId = nextMsgId++ }

  if (!packet.options) { packet.options = [] }

  if (nextMsgId === 65535) { nextMsgId = 0 }

  if (!packet.confirmable) { packet.confirmable = false }

  if (!packet.reset) { packet.reset = false }

  if (!packet.ack) { packet.ack = false }
...
function confirmableAckResetMask (packet) {
  ...
  if (packet.confirmable) { result = 0 << 4 } else if (packet.ack) { result = 2 << 4 } else if (packet.reset) { result = 3 << 4 } else { result = 1 << 4 } // the message is non-confirmable
...
function calculateLength (packet, options) {
  ...
  if (packet.code !== '0.00' && packet.payload != '') { length += 1 }
